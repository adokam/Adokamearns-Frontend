// ============================================================
//  tasks.js — Fetch, render, filter, submit tasks
// ============================================================
...
export async function loadTasks(userId) {
  ...
  try {
    ...
    grid.innerHTML = `<div class="tasks-error">⚠️ Could not load tasks. Check your connection.</div>`;
    console.error(err);
  }
}
  ...
  filtered.forEach((task, i) => {
    ...
    if (sub) {
      if (sub.status === "pending") {
        ...
        cardClass = "available";
        badgeHTML = `<span class="task-badge badge-rejected">✗ Rejected — Resubmit</span>`;
        btnHTML   = `<button class="task-btn task-btn-go" data-taskid="${task.id}">Resubmit →</button>`;
      }
    }

    const card = document.createElement("div");
    card.className = `task-card ${cardClass} fade-in`;
    ...
    if (btn) btn.addEventListener("click", () => openTaskModal(task));
...
function updateProgressBanner() {
  ...
  if (text) text.textContent   = `${done} of ${goal} tasks completed this batch`;
}
...
function openTaskModal(task) {
  ...
  document.getElementById("task-modal-desc").textContent  =
    `Click the button below to complete this task. Come back here and click "I've Done It" to submit for admin review. You'll earn ₦${Number(task.reward).toLocaleString()} once approved.`;
